...
async fn omp_handoff_closes_after_switching_to_acp_and_a_new_worktree_root() {
    ...
    fs::write(worktree.join("artifact.txt"), "done").expect("artifact");
    run_git(&worktree, ["add", "artifact.txt"]);
    run_git(&worktree, ["commit", "-m", "SYM-235 implementation"]);
    let head_sha = git_output(&worktree, ["rev-parse", "HEAD"])
        .trim()
        .to_owned();
    ...
    store.reconcile_projects(&config).await.expect("projects");
    store
        .upsert_issue(test_issue("symphony", "omp-semantic", "SYM-235"))
        .await
        .expect("issue");
    ...
    assert_eq!(
        client.transitions(),
        vec![("omp-semantic".into(), LinearTransition::Done)]
    );
    assert!(opencode.repairs().is_empty());
    let issue = store
        .issue("symphony", "omp-semantic")
        .await
        .expect("query issue")
        .expect("issue");
    assert_eq!(issue.lifecycle_stage, LifecycleStage::Completed);
    assert_eq!(
        issue.git_ref.expect("git ref").head_sha.as_deref(),
        Some(head_sha.as_str())
    );
}

#[tokio::test]
async fn omp_handoff_from_active_legacy_provider_worktree_outside_current_root_closes() {
    let dir = tempfile::tempdir().expect("tempdir");
    let db_path = dir.path().join("runtime.sqlite3");
    let repo = dir.path().join("repo");
    let origin = dir.path().join("origin.git");
    ...
    let current_root = dir.path().join("opencode-worktrees");
    let legacy_provider_root = dir.path().join("omp-worktrees");
    fs::create_dir_all(&legacy_provider_root).expect("legacy provider root");
    let worktree = legacy_provider_root.join("SYM-236");
    run_git(
        &repo,
        [
            "worktree",
    ...
    fs::write(worktree.join("artifact.txt"), "done").expect("artifact");
    run_git(&worktree, ["add", "artifact.txt"]);
    run_git(&worktree, ["commit", "-m", "SYM-236 implementation"]);
    let head_sha = git_output(&worktree, ["rev-parse", "HEAD"])
        .trim()
        .to_owned();
    let issue_branch = "symphony/SYM-236";
    let issue_refspec = format!("HEAD:refs/heads/{issue_branch}");
    run_git(&worktree, ["push", "origin", &issue_refspec]);
    ...
    store.reconcile_projects(&config).await.expect("projects");
    store
        .upsert_issue(test_issue("symphony", "legacy-omp", "SYM-236"))
        .await
        .expect("issue");
    let mut session = test_session("symphony", "legacy-omp", "omp-runtime-id", &worktree);
    session.provider_mode = RuntimeProviderMode::OmpAcp;
    session.provider_id = Some("omp-primary".into());
    store.upsert_runner_session(session).await.expect("session");
    ...
    assert_eq!(
        client.transitions(),
        vec![("legacy-omp".into(), LinearTransition::Done)]
    );
    assert!(
        client
            .evidence()
        ...
        "active OMP worktree outside current root must not be self-defected"
    );
    assert!(opencode.repairs().is_empty());
    let issue = store
        .issue("symphony", "legacy-omp")
        .await
        .expect("query issue")
        .expect("issue");
    assert_eq!(issue.lifecycle_stage, LifecycleStage::Completed);
    assert_eq!(issue.cleanup_status, CleanupStatus::Complete);
    assert_eq!(
        issue.git_ref.expect("git ref").worktree_path,
        worktree.display().to_string()
    );
    assert!(
        !worktree.exists(),
        "accepted active worktree should be cleaned up"
    );
}
...
async fn malformed_handoff_sidecar_in_review_repairs_with_review_agent() {
    ...
    assert!(
        ...
        "malformed sidecar should repair in place before parking, got {:?}",
        client.transitions()
    );
    assert!(client.evidence().iter().any(|(_, evidence)| {
        evidence.kind == "malformed_handoff"
...
async fn dead_in_progress_session_without_handoff_sidecar_fails_fast_instead_of_reusing_session() {
    ...
    assert!(
        client.transitions().is_empty(),
        "dead in-progress session should repair in place before parking, got {:?}",
        client.transitions()
    );
    assert!(client.evidence().iter().any(|(_, evidence)| {
        evidence.kind == "malformed_handoff"
...
async fn live_acp_process_with_stale_opencode_activity_fails_fast_without_resume_loop() {
    ...
    assert!(
        client.transitions().is_empty(),
        "stale live ACP process should be repaired in place before parking, got {:?}",
        client.transitions()
    );
    assert_eq!(
        opencode.repairs(),
        vec![("ses-root".into(), "missing_handoff_sidecar".into())]
    );
    let issue = store
...
async fn missing_handoff_after_local_commit_records_worktree_git_snapshot_and_process_ref() {
    ...
    assert!(
        client.transitions().is_empty(),
        "missing sidecar with local commits should repair in place, got {:?}",
        client.transitions()
    );
    let evidence = client
        .evidence()
...
async fn missing_handoff_after_dirty_worktree_records_dirty_salvage_snapshot() {
    ...
    assert!(
        client.transitions().is_empty(),
        "dirty worktree should stay in repair instead of Backlog, got {:?}",
        client.transitions()
    );
...
async fn missing_handoff_after_no_diff_records_explicit_no_change_salvage_snapshot() {
    ...
    assert!(
        client.transitions().is_empty(),
        "no-diff missing sidecar should request explicit no-change repair, got {:?}",
        client.transitions()
    );
    assert_eq!(
        opencode.repairs(),
...
async fn missing_handoff_after_unpushed_branch_records_unpushed_salvage_snapshot() {
    ...
    assert!(
        client.transitions().is_empty(),
        "unpushed branch should stay in repair instead of Backlog, got {:?}",
        client.transitions()
    );
    assert_eq!(
        opencode.repairs(),
