defmodule SymphonyElixir.Codex.AppServer do
  ...
  defp validate_workspace_cwd(workspace, nil, settings) when is_binary(workspace) do
    ...
    with {:ok, canonical_workspace} <- PathSafety.canonicalize(expanded_workspace),
      ...
      cond do
        ...
        codex_project_root_cwd?(canonical_workspace, settings) ->
          {:ok, canonical_workspace}
  ...
  defp codex_project_root_cwd?(canonical_workspace, settings) when is_binary(canonical_workspace) do
  ...
  defp session_policies(workspace, nil, settings) do
    Config.codex_runtime_settings(workspace, settings: settings)
  end

  defp session_policies(workspace, worker_host, settings) when is_binary(worker_host) do
    Config.codex_runtime_settings(workspace, remote: true, settings: settings)
  end
  ...
  defp start_thread(port, workspace, %{thread_id: thread_id} = policies, settings)
       when is_binary(thread_id) and thread_id != "" do
    resume_thread(port, workspace, policies, settings)
  end

  defp start_thread(port, workspace, %{approval_policy: approval_policy, thread_sandbox: thread_sandbox}, settings) do
    send_message(port, %{
      "method" => "thread/start",
      "id" => @thread_start_id,
      "params" => %{
  ...
  defp await_response(port, request_id, settings) do
    with_timeout_response(port, request_id, settings.codex.read_timeout_ms, "")
  end
